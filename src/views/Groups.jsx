...
import { Wrap, LivePill, Badge } from '../components/atoms.jsx'

function GroupCard({ g }) {
  const { th, t, favs, toggleFav, standings } = useStore()
  const s = standings(g)
    ...
    <div style={{ border: '1px solid ' + th.bd, background: th.sf, borderRadius: 18, overflow: 'hidden' }}>
      ...
      <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
        <thead>
          <tr>
            {head.map((c, i) => (
              <th key={i} style={{ textAlign: i < 2 ? 'left' : 'center', padding: '7px 5px', fontSize: 10.5, fontWeight: 700, color: th.faint, letterSpacing: '0.03em', width: i === 0 ? 26 : (i === 1 ? 'auto' : 30) }}>{c}</th>
            ))}
          </tr>
        </thead>
        <tbody>
          {s.rows.map(r => {
            ...
            return (
              <tr key={r.id} onClick={() => toggleFav(r.id)} style={{ cursor: 'pointer', background: fav ? th.accentSoft : 'transparent', borderTop: '1px solid ' + th.bd }}>
  ...
  return (
    <Wrap>
      <div style={{ display: 'flex', gap: 18, flexWrap: 'wrap', marginBottom: 18 }}>
        ...
        <span style={{ fontSize: 12, color: th.faint, fontWeight: 600 }}>Tap a team to follow ★</span>
      </div>
